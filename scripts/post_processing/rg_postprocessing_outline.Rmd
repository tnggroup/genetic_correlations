...
standard_error_ci <-
  # calculate the confidence interval around the point estimate (rg mean)
  get_confidence_interval(point_estimate = rg,
                          # at the 95% confidence level
                          level = .95,
                          # using the standard error
                          type = "se") #this computes lower_ci and upper_ci
...
GWAS_info_sheet %>%
  filter(
    lower_ci <= 0 # lower confidence interval of h2 estimate less than or equal to 0
    ) %>%
  nrow()

# or use h2 z score cut off of 5 or 2.5? z = rg/se
```
