...
author: "Abigail ter Kuile, Christopher Huebel, Alish Palmos, Johan Zvrskovec, Oliver Pain"
date: "28/08/2021"
...
# #JZ settings for PGC3 MDD GWAS
# column_options$file<-'/Users/jakz/Documents/local_db/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/raw/daner_pgc_mdd_full_eur_hg19_v3.49.24.11.rp.gz'
# column_options$label<-"Major depressive disorder" #This is the nice trait name
# column_options$code<-'MDD_PGC3'
# column_options$`sample-size`<-NA
# column_options$population<-'EUR'
# column_options$output<-'/Users/jakz/Documents/local_db/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/cleaned_shpipeline'
# column_options$reference_doi=NA

# #JZ settings for PGC2 MDD GWAS
# column_options$file<-'/Users/jakz/Documents/local_db/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/raw/pgc.mdd.full.2012-04.txt'
# column_options$label<-"Major depressive disorder" #This is the nice trait name
# column_options$code<-'MDD_PGC1'
# column_options$`sample-size`<-NA
# column_options$population<-'EUR'
# column_options$output<-'/Users/jakz/Documents/local_db/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/cleaned_shpipeline'
# column_options$reference_doi=NA

# #JZ settings for X chromosome test
# column_options$file<-'~/project/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/raw/Hagenaars2017_UKB_MPB_summary_results_10Feb2016.txt'
# column_options$file2<-'~/project/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/raw/Hagenaars2017_UKB_MPB_summary_results_Xchr_15Feb2017.txt'
# column_options$label<-'Male pattern baldness (androgenic alopecia)'
# column_options$code<-'BALD01'
# column_options$`sample-size`<-52874
# column_options$population<-'EUR'
# column_options$sex<-'both'
# #column_options$output<-'/scratch/groups/gwas_sumstats'
# column_options$output<-"~/project/JZ_GED_PHD_ADMIN_GENERAL/data/gwas_sumstats/cleaned_shpipeline"
...
**NOTE: for testing you can specify below to just read in first 10k rows of SNPs to run tests faster - uncomment the command**
```{r Read in sumstats}
cCode <- traitCodes[1]
...
cSumstats <- data.table::fread(
  file = cFilePath,
  header = T, #Interpret the first row as column names
  quote = "\"",
  na.strings = c(
    ".",
    NA,
  ...
  #check.names = T, #JZ: do we want this? - checks duplicates and variable naming syntax
  fill = T, #Fills empty values with NA based on interpreted column names
  blank.lines.skip = T, #skips empty lines
  data.table = T, #convert to data table immediately
  nThread = 5 #uses 5 threads rather than one
  # For testing read in first 10k rows
  # nrows=10000
  )

# Temporary code to read in the secod file from the Hagenaars2017_UKB_MPB_summary_results sumstats
# cSumstats2 <- data.table::fread(
#   file = column_options$file2,
#   header = T,
#   quote = "\"",
#   na.strings = c(
#     ".",
...
# cSumstats<-rbindlist(list(cSumstats,cSumstats2),use.names = F)
# names(cSumstats)<-names(cSumstats2)
# rm(cSumstats2)

#remove empty rows
naRows <- rowSums(is.na(cSumstats)) == length(colnames(cSumstats))
cat("\nNumber of completely NA rows:",sum(naRows),"\n")
if(sum(naRows)>0) cSumstats<-cSumstats[!naRows,]
rm(naRows)


# Number of rows before touching the data
sumstats_meta[cCode,c("n_snp_raw")]<-nrow(cSumstats) #add number of SNPs raw to sumstats_meta (meta data file)
...
cSumstats <- setDT(cSumstats) #JZ: Slightly redundant as DT is set by fread above now - remove?
str(cSumstats)
```
...
**JZ: BGENIE FILE FORMAT SHOULD BE TAKEN CARE OF BY THE COLUMN PARSING FUNCTION - IF NOT -> CORRECT IN THE FUNCTION**
...
+++JZ: Needs to be fixed? Ratio ca/co should not be stored in FRQ as this may be used for effect allele frequency. I encountered a file where ncas and ncon were provided per variant.
...
if(!any(colnames(cSumstats)=="FRQ") & 
  ...
  if(!is.na(danerNcas) & !is.na(danerNcon))
    ...
    # Add number of cases and number of controls - but only to already NA fields! The file may already contain N_CAS and N_CON columns.
    cSumstats[is.na(cSumstats$N_CAS),]$N_CAS <- as.numeric(gsub('FRQ_A_','',danerNcas))
    cSumstats[is.na(cSumstats$N_CON),]$N_CON <- as.numeric(gsub('FRQ_U_','',danerNcon))
    ...
    cat("The dataset is not a daner file")
  }
  }
```


**JZ: We already have CHR columns as integers from chunks above**
...
if(
  ...
  cat("N set from N_CAS and N_CON value")
  head(cSumstats)
  } else if(!is.null(cN)) {
...
if(
  ...
  sumstats_meta[cCode,c("Effect")]<-"OR"
    ...
    sumstats_meta[cCode,c("Effect")]<-"Z"
  } else if(any(colnames(cSumstats) == "P")){
    sumstats_meta[cCode,c("Effect")]<-"P"
  } else {
    sumstats_meta[cCode,c("Effect")]<-NA_character_
  }
...
**JZ: We don't have BETA if the effect was only provided in Z**
...
+++JZ: We could add some more controle over what to propritise when removing duplicates here: MAF?
...
+++JZ: I think we also need to re-adjust BETA, SE and Z accordingly after correction #TODO
...
+++JZ: This does not seem to be working for indel alleles - maybe only use the first insertion base?
...
+++JZ: We could add SNP counts from the different operations here to the metadata table
+++JZ: The build comparison will fail if chromosome is different across builds. Relevant for sex chromosomes.
+++JZ: The reference panel chromosome naming in the files does not correspond to our numeric chromosomes.
```{r merge with reference}

#for testing
#cSumstatsOLD<-cSumstats
#cSumstats<-cSumstatsOLD[CHR==1 | CHR==23,]
#cSumstats<-cSumstatsOLD[CHR==1 | CHR==2,]
...
if('FRQ' %in% names(cSumstats_harm)){
  if(any(cSumstats_harm$FRQ>1, na.rm = T) || any(cSumstats_harm$FRQ<0, na.rm = T)) {
...
sumstats_meta[cCode,c("frq_bad")] <- sum(cSumstats_harm$FRQ_bad)
...
fwrite(cSumstats_harm, paste0(column_options$output,"/cleaned/", cCode, ".gz"), sep='\t', na='NA', quote=F, nThread = 3)
...
sumstats_meta %>%
  write_tsv(
    file = paste0(column_options$output,"/cleaned_logs/", cCode, "_log.tsv"),
    col_names = TRUE
    )
```


Chunk to update a Google spreadsheet - for demo purposes

```{r update google spreadsheet, eval=F}
