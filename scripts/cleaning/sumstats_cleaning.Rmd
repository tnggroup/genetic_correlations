...
if(!any(colnames(cSumstats)=="FRQ") & 
   any(startsWith(colnames(cSumstats),prefix = "FRQ_A_")) & 
   any(startsWith(colnames(cSumstats),prefix = "FRQ_U_")))
  {
...
if(any(colnames(cSumstats)=="N_CAS") && any(colnames(cSumstats)=="N_CON")) {
  ...
  head(cSumstats)
    ...
    cSumstats$N<-NA
    cSumstats[is.na(cSumstats$N),]$N <- cN #default behaviour: add explicit N only to missing values.
...
+++JZ: We could add some more control over what to propritise when removing duplicates here: MAF?
...
fwrite(cSumstats_harm, paste0(column_options$output,"/cleaned/", cCode, ".gz"), sep='\t', na='NA', quote=F, nThread = 5)
