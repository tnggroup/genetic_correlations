...
+++CH: Yes, please convert CHR to numeric
...
# MAF and frequency columns

NOTE: This would only work if in the harmonization step earlier, the MAF_REF column was inserted into the cSumstats dataframe (so need to add harmonisation step before this step)

+++Alish: this is slightly different to what we were previously doing in Helena's script. Discuss in meeting?
+++CH: Did we import the reference? I'm confused.
...
if(
  any(cSumstats$FRQ > 1) || any(cSumstats$FRQ < 0)
  ) {
  
  stop(paste0('\nThere are FRQ values larger than 1 (',
              sum(cSumstats$FRQ>1),
              ') or less than 0 (',
              sum(cSumstats$FRQ<0),
              ') which is outside of the possible FRQ range.')
  )
}
...
if(
  !is.null(cond.invertedAlleleOrder) 
  & !harmoniseAllelesToReference) 
  {
