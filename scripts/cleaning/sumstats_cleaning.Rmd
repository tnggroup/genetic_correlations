...
THIS IS WHERE YOU SET UP YOUR DATASET SETTINGS! ENTER THE SETTINGS FOR YOUR DATASET AS WAS DONE FOR THE ADHD EXAMPLE GWAS BELOW.

+++JZ: These settings are temporary and if we want to convert the script to a function and a command line utility we will have to adapt them accordingly.

```{r}
filePaths <- clOptions$file #a list of file paths you want to be processed in parallel. in the function this will be a list, but for the command line utility we have to parse the argument, maybe have paths separated by comma or similar?
traitNames <- clOptions$code # a list of nice trait names which could be fit for a publication table/graph/plot
traitCodes <- clOptions$code # a list of codes you want to identify your traits by (different from the nice names), used to reference a trait by a string
#test or temporary placeholders
filePaths<-c("../../data_raw/CUD_EUR_full_public_11.14.2020.gz") #SET YOUR FILE PATH, when testing
traitNames<-c("Cannabis use disorder (2020)") #SET YOUR NICE TRAIT NAME, when testing
traitCodes<-c("CUDI01") #SET YOUR TRAIT CODE STRING, when testing
...
+++CH: to read in the data file, I would use the data.table package because it is much faster than the basic R functions to read in data
https://cran.r-project.org/web/packages/data.table/vignettes/datatable-intro.html
```{r read in sumstats}
filePaths <- clOptions$file
#test
filePaths<-c("../../data_raw/CUD_EUR_full_public_11.14.2020.gz")
...
DO WE WANT TO CONVERT CHR TO NUMERIC VALUES? ^
...
+++JZ: I moved this here so we can rely on BETA existing if either BETA or OR. We can't know if the effect is a non-OR unless we check though.
...
if(any(colnames(cSumstats) == "SE") == 0){ 
  ...
  #+++ATK not too sure if we need to add to meta data? or if not necessary? - yes. set SE_from_Z as column and set as T
...
sumstats.meta[cCode,c("enough_columns_PRS")]<-any(colnames(cSumstats)=="SNP") & any(colnames(cSumstats)=="P") & (any(colnames(cSumstats)=="BETA") | any(colnames(cSumstats)=="OR") | any(colnames(cSumstats)=="Z") ) & any(colnames(cSumstats)=="A1") & any(colnames(cSumstats)=="A2")
sumstats.meta[cCode,c("enough_columns_pathwayanalysis")]<-any(colnames(cSumstats)=="SNP") & any(colnames(cSumstats)=="P")
