...
+++CH: Maybe we can annotate this a little more?
```{r}
...
```
+++ATK add z column here

+++ add BETA column here 
+++ add SE column here
...
#Missing effect standard error
Add standard errors from beta & z or or & z (Ollies script)
+++ATK: move above where N column is added and add Z chunk 
...
if(any(colnames(cSumstats) == "SE") == 0){ 
  if(any(colnames(cSumstats) == "BETA") == 1){ #take BETA if BETA column exists
    cSumstats$Z<-abs(qnorm(cSumstats$P/2)) #calculate Z value using qnorm function - we shouldn't need this step if we are including chunk 
    cSumstats$SE<-abs(cSumstats$BETA/cSumstats$Z) #calculate SE from BETA and Z
  } else {
    cSumstats$Z<-abs(qnorm(cSumstats$P/2)) #calculate Z value
    cSumstats$SE<-abs(log(cSumstats$OR)/cSumstats$Z) #if no BETA column, calculate SE from odds ratio and Z
  }
  
  cSumstats$Z<-NULL ###do we want to name Z something else here so we don't over write Z values? e.g. Z_for_SE?
...
+++ATK go over in next meeting what to add from Helenas
Need to add from Helenas:
Helena: /mnt/lustre/groups/ukbiobank/sumstats/scripts/computeLambdaAndCorrect.r

Added from Ollie's:

Need to calculate BETA 
```{r check and correct for genomic control}
if(any(colnames(cSumstats) == "SE") == 1){
  if(any(colnames(cSumstats) == "OR") == 1){
    cSumstats$BETA<-log(cSumstats$OR) ####change to BETA_check?
