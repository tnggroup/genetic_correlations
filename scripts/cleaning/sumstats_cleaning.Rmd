...
+++ATK: Please check in next meeting
...
if(any(colnames(cSumstats) == "SE") == 0){ 
  if(any(colnames(cSumstats) == "BETA") == 1){ #take BETA if BETA column exists
    cSumstats$Z<-abs(qnorm(cSumstats$P/2)) #calculate Z value using qnorm function
  ...
  cSumstats$Z<-NULL ###do we want to name Z something else here so we dont over write Z values? e.g. Z_for_SE?
  
  cat("SE column inserted based on BETA/OR and P")
  
  #+++ATK not too sure how best to add to meta data? or if not necessary?
  sumstats.meta[cCode,c("SE_column_based_on_BETA/OR_and_P")]<-T
  }
```
...
##++++ATK: what do we want to add from Helena's below?
